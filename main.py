"""
Pose keypoints demo for a stretching app (PC).

Requirements:
    pip install mediapipe opencv-python numpy

Run:
    python pose_stretch_demo.py
"""
import cv2
...
def keypoints_to_dict(landmarks, image_shape):
    ...
    for lm_name in names:
        ...
        kp[lm_name.name] = {"x": x, "y": y, "visibility": lm.visibility if hasattr(lm, "visibility") else None}
    return kp
    ...
    while True:
        ...
        if results.pose_landmarks:
            ...
            for i, text in enumerate(feedback):
                cv2.putText(frame, text, (10, y0 + i*20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
